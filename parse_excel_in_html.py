...
'''
htmlからリンクを抽出し、そのリンクからexcelファイルをダウンロード、ダウンロードしたexcelファイルを解析するプログラムのサンプルです。
例外処理などは殆どしていない ＆ 読み取るexcelデータの形式が決まっていますが、適宜変更を加えて、自分のプログラムに組み込んで下さい。

このプログラムが読み取る対象としたサイトは、農林水産省の輸入データを統計としてまとめた、
